...
import type { Preferences, ModuleError } from 'codesandbox/common/types';

import { getCodeMirror } from 'codesandbox/app/utils/codemirror';
...
export default class CodeEditor extends React.Component<Props, State> {
  ...
  setCodeMirrorPreferences = async () => {
    ...
    if (preferences.lintEnabled) {
      ...
      System.import('codesandbox/app/utils/codemirror/eslint-lint')
  ...
  prettify = async () => {
    ...
    if (mode === 'jsx' || mode === 'typescript' || mode === 'css') {
      try {
        const prettify = await import('codesandbox/app/utils/codemirror/prettify');
